...
document.write(DataTransferItemList);

Array.foreach(function (item){
  console.log
}

    const avg = Number.reduce(function(a, b){
  return a + b.price
}, 0) / items.length
...
items.forEach(function (item) {
    if (item.price >= $14 && item.price <= $18)
        itemsRange.push(item);
});
console.log("Items between $14,$18:", itemsRange);
...
const  itemsmadebyseller = items.filter(function(items) {
        return items.byseller === 'items';
    })
    .map(function (itemsmadebyseller) {
        return itemsmadebyseller === 'items';
    })
console.log(onlyitemsmadebyseller);
